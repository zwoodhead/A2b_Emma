...
#Updated by Paul Thompson - 15th September 2020 based on reviewer comments. 
```
...
library(OpenMx)
library(umx)
library(semTools) #for fit measures
library(data.table)
library(devtools)
library(reshape2)
library(magrittr)
...
# # Is bilaterality LESS stable than left or right lateralisation?
# 
# stability <- matrix(NA, nrow = 6, ncol = 6)
# colnames(stability) <- myshortlab
# for (t in 1:6){
#   # Left lateralised
#   stability[1, t] <- length(which(alltasklat[ , t] == 3 & lat_consistency[ , t] == 1))
#   stability[2, t] <- length(which(alltasklat[ , t] == 3 & lat_consistency[ , t] == 0))
#   # Bilateral
#   stability[3, t] <- length(which(alltasklat[ , t] == 2 & lat_consistency[ , t] == 1))
#   stability[4, t] <- length(which(alltasklat[ , t] == 2 & lat_consistency[ , t] == 0))
#   # Right lateralised
#   stability[5, t] <- length(which(alltasklat[ , t] == 1 & lat_consistency[ , t] == 1))
#   stability[6, t] <- length(which(alltasklat[ , t] == 1 & lat_consistency[ , t] == 0))
# }
...
```{r behaviouraldata}
osf_retrieve_file("https://osf.io/wznbv") %>% osf_download(conflicts = "skip") # A2_Behavioural_Data.csv
behavdat_A2    <- read.csv("A2_Behavioural_Data.csv")
osf_retrieve_file("https://osf.io/mzgan") %>% osf_download(conflicts = "skip") # A2_Behavioural_Data_Bangor.csv
behavdat_Bangor    <- read.csv("A2_Behavioural_Data_Bangor.csv")

colnames(behavdat_A2)[1] <- "ID"
colnames(behavdat_Bangor)[1] <- "ID"
...
# Check / download script for ggpirate plot
osf_retrieve_file("https://osf.io/26w9g/") %>% osf_download(conflicts = "overwite") # Downloads geom_pirate_ZW.R
source("geom_pirate_ZW_local.R")
...
## Oxford vs Bangor pirate plot

This chunk creates a pirate plot similar to the one above, but where the point shape indicates whether participants are from Oxford vs Bangor.
...
ID_data <- data.frame(
  ID = as.factor(particdat$ID),
  Hand = as.factor(particdat$handedness),
  Site = as.factor(particdat$study))
...
my_pirate = ggplot(data = LI_long,
 ...
 geom_pirate_ZW(points_params = list(alpha = 0),
  ...
  theme(legend.position = "right", panel.grid.minor = element_blank(), panel.grid.major.x = element_blank()) +
  geom_hline(yintercept = 0) +
  facet_grid(Session~Hand) +
  # Coded points for laterality categories
  geom_point(data = LI_long, aes(x = Taskcode, colour = Site, y = LI),
             position = position_jitterdodge(jitter.width = 0.4, dodge.width = 0.7),
             size = 1.2, alpha = 0.5, na.rm = TRUE) +
  scale_colour_manual(values = c(1, 2), 
                     name = "Testing Site", labels = c("Oxford", "Bangor"))

print(my_pirate)

ggsave(filename = "Supplementary_Figure2.png", plot = my_pirate, width = 8, height = 6)
...
## LI Split-half reliablity

This chunk calculates the split-half reliability using odd and even trials

```{r LI-splithalf}
# Select odd and even data
allodd <- cbind(dplyr::select(data1, A1.mean_odd, B1.mean_odd, C1.mean_odd, D1.mean_odd, E1.mean_odd, F1.mean_odd),
                dplyr::select(data2, A2.mean_odd, B2.mean_odd, C2.mean_odd, D2.mean_odd, E2.mean_odd, F2.mean_odd))
alleven <- cbind(dplyr::select(data1, A1.mean_even, B1.mean_even, C1.mean_even, D1.mean_even, E1.mean_even, F1.mean_even),
                 dplyr::select(data2, A2.mean_even, B2.mean_even, C2.mean_even, D2.mean_even, E2.mean_even, F2.mean_even))

# Test-retest comparisons of LI values
LI_splithalf <- data.frame(matrix(NA, nrow=24, ncol=5))
colnames(LI_splithalf)<-c("Hand", "Task", "Session", "r", "p")
LI_splithalf$Hand <- rep(c(1,2),each=12)
LI_splithalf$Session <- rep(c(1,2), each=6, 2)
LI_splithalf$Task <- rep(myshortlab, 4)
...
for (h in 1:2) {
  ...
  for (mycol in 1:12){
    ...
    my_test <- cor.test(allodd[handedness_rows, mycol],    # Odd trials
                      alleven[handedness_rows, mycol])  # Even trials
    # Organise output
    LI_splithalf$r[myrow] <- round(my_test$estimate, 2)
    LI_splithalf$p[myrow] <- round(my_test$p.value, 3)
  }
}
...
#PT edit - sept2020 - added BIC values to satisfy reviewer 3. All other requested information is presetn in mxCompare output.
myBIC <- vector(length = 4)
myBIC[1] <- umxFitIndices(Model1LH)$informationCriteria$deviance[2,1]
myBIC[2] <- umxFitIndices(Model2LH)$informationCriteria$deviance[2,1]
myBIC[3] <- umxFitIndices(Model1RH)$informationCriteria$deviance[2,1]
myBIC[4] <- umxFitIndices(Model2RH)$informationCriteria$deviance[2,1]
cat('BIC values for Table 4: \n')
cat(myBIC)
...
#=====================================================================#
Leftfs = umxFactorScores(Model2LH_con, type = 'Regression', minManifests = 0)
Rightfs = umxFactorScores(Model2RH_con, type = 'Regression', minManifests = 0)

alltask$f1scoreNew <- 0
alltask$f2scoreNew <- 0
alltask$f1scoreNew[LH_row] <- Leftfs$F1
alltask$f2scoreNew[LH_row] <- Leftfs$F2
alltask$f1scoreNew[RH_row] <- Rightfs$F1
alltask$f2scoreNew[RH_row] <- Rightfs$F2


alltask$cookdistNew <- 0
facmod_LH <- lm(alltask$f1scoreNew[LH_row] ~ alltask$f2scoreNew[LH_row])
facmod_RH <- lm(alltask$f1scoreNew[RH_row] ~ alltask$f2scoreNew[RH_row])
alltask$cookdistNew[LH_row] <- cooks.distance(facmod_LH)
alltask$cookdistNew[RH_row] <- cooks.distance(facmod_RH)
#=====================================================================#

plot(alltask$cookdistNew, col=particdat$handcode, pch=16, ylab = "Cooks Distance", xlab = "Participant")
w<-which(alltask$cookdistNew>(4*mean(alltask$cookdistNew)))
abline(h=4*mean(alltask$cookdistNew), lty=2)
abline(v=37.5, lty=2)
...
png("Figure6New.png", width=6, height=5, units="in", res=300)
par(mar = c(4.5, 4.1, 1, 1))
plot(alltask$f1scoreNew,alltask$f2scoreNew, pch=alltask$bivoutlier, col=particdat$handcode, 
     xlab="Factor 1",ylab="Factor 2")
abline(v=0,lty=2)
abline(h=0,lty=2)
mycor_RH <- round(cor(alltask$f1scoreNew[RH_row], alltask$f2scoreNew[RH_row], use="pairwise.complete.obs"), 3)
mycor_LH <- round(cor(alltask$f1scoreNew[LH_row], alltask$f2scoreNew[LH_row], use="pairwise.complete.obs"), 3)
text(-14, 13, paste("Right Handers: r = ",mycor_RH))
text(-14, 20, paste("Left Handers: r = ",mycor_LH), col="red")
pData<-cbind(alltask,particdat)
covEllipses(x=pData[,c("f1scoreNew","f2scoreNew")], group=pData$handcode, 
