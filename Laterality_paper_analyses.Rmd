...
cat(paste('Dropped subjects because > 2 missing or dropped datapoints, N = ', length(drop_subj)))
cat(drop_subj)
...
cat(paste('Dropped subjects because > 2 outlier datapoints, N = ', length(drop_subj_outliers)))
cat(drop_subj_outliers)
...
tab1a<- stargazer(behavdat[,word.cols],type='text',title="Word data")

#accuracy data
acc.cols<-grep('*.Acc', colnames(behavdat))
cat('Percentages correct')
stargazer(behavdat[,acc.cols],type='text',title="Percentages Correct")

#RT data
rt.cols<-grep('*.RT', colnames(behavdat))
cat('RT')
stargazer(behavdat[,rt.cols],type='text',title="RT")

#omit data
omit.cols<- grep('*.Omit',colnames(behavdat))
cat('Omit')
stargazer(behavdat[,omit.cols],type='text',title="Omit")
...
if (length(which(behav_tests$p < corrected_alpha)) == 0){
  cat(paste0('No significant difference between left and right handers observed for number of words, RT or Accuracy on any task'))
...
#ggsave(filename = 'My_factorboth.png', dpi=300, plot = my_factor2, width = 6, height = 2.5) 
```
