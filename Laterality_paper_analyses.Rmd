...
## Introduction
This script reads in data from A2 study and new data acquired at Bangor, and performs the analyses reported in the Laterality paper.
...
RHonly            <- 0 # Set to 0 if want to include both R and L handers. 
                        # Set to -1 for just L handers. MANUSCRIPT SETTING = 0
...
# RHonly: only right handed participants
if (RHonly==1){ 
  print('R handers only included in analysis')
  particdat <- filter(particdat,handedness=='R')
}

# RHonly = -1: only left handed participants
if (RHonly== -1){ 
  print('L handers only included in analysis')
  particdat <- filter(particdat,handedness=='L')
}

# Fonly: only female participants
if (Fonly== 1){
  print('Females only included in analysis')
...
# Only take the participants included in particdat (allows you to drop subjects by handedness)
...
print(paste('Dropped datapoints because < 10 trials, N = ',dropN))
...
print(paste('Dropped subjects because > 2 missing or dropped datapoints, N = ', length(drop_subj)))
print(drop_subj)
...
print(paste('Dropped datapoints because high SE, N = ',dropSE))
...
print(paste('Dropped subjects because > 2 outlier datapoints, N = ', length(drop_subj_outliers)))
print(drop_subj_outliers)
...
gender_info <- particdat %>% group_by(Gender) %>% summarise(n=n())
gender_by_study <- particdat %>% group_by(study, Gender) %>% summarise(n=n()) 
...
age_by_study <- particdat %>% group_by(study) %>% summarise(age_mean = mean(Age_m)/12, age_sd = sd(Age_m)/12)

# Do left / right handed participants differ in gender?
if (RHonly == 0){
chisq.test(gender_table)
age_by_handedness <- particdat %>% group_by(handedness) %>% 
  summarise(age_mean = mean(Age_m)/12, age_sd =sd(Age_m)/12)
# Do left / right participants differ in age
t.test(particdat$Age_m[which(particdat$handcode==1)], particdat$Age_m[which(particdat$handcode==2)])
}
print(age_info)
...
nevents <- c(15,90,90,15,90,45,15,90,90,15,90,45) # The number of events in each condition
...
for (i in 1:length(cols_to_test)){
  my_test <- t.test(behavdat[which(particdat$handcode == 1), cols_to_test[i]], behavdat[which(particdat$handcode == 2), cols_to_test[i]], paired = FALSE)
...
print(paste0('Alpha corrected for ', length(cols_to_test), ' multiple comparisons = ', corrected_alpha))

if (length(which(behav_tests$p < corrected_alpha)) > 0){
  print(paste0('Significant difference between left and right handers observed for: '))
  print(behav_tests$Measure[which(behav_tests$p < corrected_alpha)])
}
if (length(which(behav_tests$p < corrected_alpha)) == 0){
  print(paste0('No significant difference between left and right handers observed for number of words, RT or Accuracy on any task'))
  print(paste0('p > ', round(min(behav_tests$p), 4), ' in all cases.'))
}
}
```
...
LI_ttests<-data.frame(matrix(NA,nrow=24,ncol=8))
colnames(LI_ttests)<-c('Taskcode','Session','Hand','t','df','p','LI','lab')
...
my_pirate = ggplot(data = LI_long,
                   mapping = aes(x = Taskcode,
                                 y = LI,
                                 colour = Session)) +
 geom_pirate_ZW(points_params = list(shape = 16, alpha = 0.5),
                 bars = FALSE,
                 #bars_params = list(width = 3),
  ...
  # Red points for outlier datapoints
  geom_point(data = NA_long, aes(x = Taskcode, group = Session, y = LI),
             position = position_dodge(0.7), colour = 'black', size = 0.8)
...
```{r dropone, message=FALSE, warning=FALSE,echo=FALSE}
...
for (group in 1:2){
  ...
  for (thisdrop in 1:nsubs){
    ...
    myModel1 <- umxRAM("myModel1", data=dataRaw, 
                       factor_loadings_1, factor_variances_1, residual_variances, indicator_means, factor_means_1)
    summary.Model1 <- summary(myModel1)
    ...
    myModel2 <- umxRAM('myModel2', data=dataRaw,
                       ...
                       residual_variances, indicator_means, factor_means_2)
    summary.Model2 <- summary(myModel2)                  
    
    #Compare Models
    mcomp <- mxCompare(myModel2, myModel1)
...
Factor1_plot_data = data.frame(
  'Task' = rep(c('LG','PD','SD','SG','SC','JD'), 2),
...
Factor1_plot_data$Task <- factor(Factor1_plot_data$Task, levels = c('LG','PD','SD','SG','SC','JD'))
Factor1_plot_data$Group <- factor(Factor1_plot_data$Group)

#=================================================================================#
# Plot Factor 1 loadings
my_factor1 <- ggplot(Factor1_plot_data, aes(x = Task, y = Loading, fill = Task)) +
  geom_boxplot(lower = Factor1_plot_data$lowCI, upper = Factor1_plot_data$highCI) +
  ylim(-1,2) +
  theme_bw() +
  geom_hline(yintercept = 0) +
  ylab('Factor 1 Loadings') +
  facet_grid(.~Group)

print(my_factor1)
ggsave(filename = 'My_factor1.png', dpi=300, plot = my_factor1, width = 6, height = 2.5)  
...
Factor2_plot_data$Task <- factor(Factor2_plot_data$Task, levels = c('LG','PD','SD','SG','SC','JD'))
Factor2_plot_data$Group <- factor(Factor2_plot_data$Group)

#=================================================================================#
# Plot Factor 2 loadings
my_factor2 <- ggplot(Factor2_plot_data, aes(x = Task, y = Loading, fill = Task)) +
  geom_boxplot(lower = Factor2_plot_data$lowCI, upper = Factor2_plot_data$highCI) +
  ylim(-1,2) +
  theme_bw() +
  geom_hline(yintercept = 0) +
...
print(my_factor2)
ggsave(filename = 'My_factor2.png', dpi=300, plot = my_factor2, width = 6, height = 2.5) 
```
...
text(-10,20,paste('Right Handers: r = ',mycor_RH))
text(-10,13,paste('Left Handers: r = ',mycor_LH), col='red')
...
if(RHonly==0){
  ...
  t.test(mydiff~particdat$handcode)
  plot(mydiff~particdat$handcode)
  
  # Which other tasks vary with handedness?
  senttot<-(alltaskall$SentGen1+alltaskall$SentGen2)/2
  listtot<-(alltaskall$ListGen1+alltaskall$ListGen2)/2
  semtot<-(alltaskall$SemDec1+alltaskall$SemDec2)/2
  phontot<-(alltaskall$PhonDec1+alltaskall$PhonDec2)/2
  comptot<-(alltaskall$SentComp1+alltaskall$SentComp2)/2
  jabbertot<-(alltaskall$Jabber1+alltaskall$Jabber2)/2
  t.test(senttot~particdat$handcode)
  plot(senttot~particdat$handcode)
  t.test(listtot~particdat$handcode)
  plot(listtot~particdat$handcode)
  t.test(semtot~particdat$handcode)
  plot(semtot~particdat$handcode)
  t.test(phontot~particdat$handcode)
  plot(phontot~particdat$handcode)
  t.test(comptot~particdat$handcode)
  plot(comptot~particdat$handcode)
  t.test(jabbertot~particdat$handcode)
  plot(jabbertot~particdat$handcode)
  
  # Do factor scores vary with handedness?
  t.test(alltask$f1score~particdat$handcode)
  t.test(alltask$f2score~particdat$handcode)
  
  # Count how many tasks a subject is left lateralised for
  for (i in 1:nrow(alltask)){
    alltask$Nleftlat[i]<-length(which(c(listtot[i],phontot[i],semtot[i],senttot[i],comptot[i],
                                        jabbertot[i])>0))
  }
  
  # Does the number of left lateralised tasks per participant vary with handedness?
  table(alltask$Nleftlat,particdat$handcode)
  t.test(alltask$Nleftlat,particdat$handcode)
