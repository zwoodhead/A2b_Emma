...
# ID values < 100 were from Oxford
# ID values > 100 were from Bangor
all_subjects <- triallist$Filename

# Some subjects are omitted as they did not return for session 2, or it was not possible to find a clean signal
# Their data will not be analysed, but the data has been saved for clarity
excluded_subjects <- which(is.na(triallist$A2.1))
included_subjects <- all_subjects[-excluded_subjects]
nsubj <- length(included_subjects)
# Remove excluded subjects from triallist too
triallist <- triallist[-excluded_subjects, ]
...
for (mysub in 39:length(included_subjects)){    
  ...
  for (session in 1:2){
    ...
    triallist$Filename<-as.character(triallist$Filename) # unfactor this column to avoid later difficulties
    triallist <- triallist[-excluded_subjects, ]
    for (t in 1:6) {
      mycomment <- NA
      task <- tasks[t]
