...
simLI<-function(sim_type='B',npartic=1000,mymean=0,ntrial=15,TRT=0.7)
{
  library(matrixStats)
...
LItrial <- MASS::mvrnorm(ntrial,c(mymean,mymean),Sigma=matrix(c(1,TRT,TRT,1),2,2,byrow=TRUE)) #ntrial random normal deviates with mean mymean, SD = 1, and corr = TRT (see arguement inputs.
...
means=c(0,0.2,0.4)
mysimA <- simLI(sim_type='A',npartic=1000,mymean=0,ntrial=15,TRT=0.7)


for(i in means)
{
  nam<-paste0("mysimB_",i)
assign(nam,simLI(sim_type='B',npartic=1000,mymean=i,ntrial=15,TRT=0.7))
}
